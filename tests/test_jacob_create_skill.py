...
class SkillToolTests(unittest.TestCase):
    ...
    def write_skill(
        ...
        name: str = "test-skill",
        *,
        extra_frontmatter: str = "disable-model-invocation: true\n",
    ...
    def test_unfinished_scaffold_fails_validation(self) -> None:
        ...
        self.assertIn(
            "disable-model-invocation: true", (skill_dir / "SKILL.md").read_text()
        )

    def test_strict_core_and_codex_scaffolding(self) -> None:
        self.assertEqual(
            self.run_init(
                "portable", "--dir", str(self.root), "--strict-core", "--codex"
            ),
            0,
        )
...
"""
    ...
    def test_core_profile_rejects_vendor_frontmatter(self) -> None:
        errors, _ = validate_skill.validate(self.write_skill(), profile="core")
        self.assertTrue(
