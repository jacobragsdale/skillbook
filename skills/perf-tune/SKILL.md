...
- Make no edits before the user approves specific plan items.
...
2. An existing benchmark suite or bench script in the repo.
...
debug builds profile a different program.
...
Record the baseline wall and CPU medians and the noise floor (the B/A interval
width). Output must be identical across these runs. If `abtest.py` exits 4,
the output is nondeterministic; find a normalization (sort, strip timestamps,
fixed seed) that makes it stable before going further.

### 3. Profile

Take a CPU profile and a syscall summary of the workload (tools in
`references/measurement.md` **Profilers**). Classify the workload from the
baseline: CPU time well below wall means it waits on IO or locks; CPU near
wall means compute; CPU above wall means already parallel.
...
house-standards skills). Run the test suite once now and record its result.
...
workload and profilers. Skip a lens the classification rules out (no `io`
for a pure in-memory compute kernel) and say you did.
...
that fail, merge duplicates, and when two leads conflict, keep one and note
the trade-off.
...
- **Needs a decision:** anything that adds a dependency, changes a public
  API, stored or wire format, durability, or result order. Include these in
  the plan but not in "all" approvals unless the user names them.

Present the plan in chat in this format. Every field is REQUIRED:

```markdown
## Perf plan — <repo> (<workload>)

Baseline: <wall> ms wall, <cpu> ms CPU, noise floor ±<x>%. <one sentence on where time goes>.

### Do first
1. **<imperative title>** `[io]` · est −<ms> ms (−<pct>%) · conf H · effort S · risk: none
   `src/load.py:88` — <evidence: profile share + code fact, one sentence>
...
<n> leads from <k> lenses; <m> dropped after verification.
...
commands run the main checkout, and every A/B then reads "no difference".
Prove it once: print the resolved module path or binary path from each side.
...
2. Rebuild and run the tests that cover the touched code.
...
4. Prove equivalence with every layer the item's risk needs, from
   ...
   stdout on the workload input only.
...
6. Commit a kept item, then move the baseline to it (`git -C <tmp>/perf-base
   checkout --detach <sha>`) and rebuild, so each item is measured against
   the items kept before it. When the repo's instructions don't ask for
   commits, these are temporary: step 7 undoes them.

### 7. Report

Measure the end-to-end change: check the baseline worktree out at the start
commit, rebuild, and A/B it against the final state with the same workload.
Remove the worktree (`git worktree remove`). If the commits were temporary,
`git reset --soft <start commit>` to leave the kept changes uncommitted.
